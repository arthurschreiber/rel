...
describe 'Integrating rel', ->
  ...
  it 'should perform a users find', ->
    ...
    assert.equal users.where(users.column('name').eq('amy')).toSql(@engine), 'SELECT FROM "users" WHERE "users"."name" = \'amy\''

  it 'should run through the first example on the readme', ->
    users = new Rel.Table 'users'
    assert.equal users.project(Rel.star()).toSql(@engine), 'SELECT * FROM "users"'
