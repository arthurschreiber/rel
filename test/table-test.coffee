...
describe 'Table', ->
  ...
  it 'should return an insert manager', ->
    ...
    assert.equal im.toSql(@engine), 'INSERT INTO "users" VALUES(NULL)'

  describe 'skip', ->
    it 'should add an offset', ->
      sm = @relation.skip 2
      assert.equal sm.toSql(@engine), 'SELECT FROM "users" OFFSET 2'

  describe 'having', ->
    it 'adds a having clause', ->
      mgr = @relation.having @relation.column('id').eq(10)
      assert.equal mgr.toSql(@engine), 'SELECT FROM "users" HAVING "users"."id" = 10'
  ...
  describe 'group', ->
    it 'should create a group', ->
      ...
      assert.equal mgr.toSql(@engine), 'SELECT FROM "users" GROUP BY "users"."id"'
  ...
  describe 'order', ->
    it 'should take an order', ->
      ...
      assert.equal mgr.toSql(@engine), 'SELECT FROM "users" ORDER BY foo'

  describe 'take', ->
    it 'should add a limit', ->
      mgr = @relation.take 1
      mgr.project new SqlLiteral('*')
      assert.equal mgr.toSql(@engine), 'SELECT  * FROM "users" LIMIT 1'

  describe 'project', ->
    it 'can project', ->
      mgr = @relation.project new SqlLiteral('*')
      assert.equal mgr.toSql(@engine), 'SELECT * FROM "users"'
  ...
  describe 'where', ->
    it 'returns a tree manager', ->
      ...
      assert.equal mgr.toSql(@engine), 'SELECT "users"."id" FROM "users" WHERE "users"."id" = 1'
